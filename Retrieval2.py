...
def retrieval(final_grid,input_grid,state_grid,target_block,path,ppo,step,grids,blocks,rewards,actions,dones,masks,probs,lookahead_num,TP_type_len):
    do=0
    stock=[]
    obstacles_set=[]
    for st in path:
    ...
    return rearrange_num,state_grid,step,grids,blocks,actions,rewards,dones,masks,probs
...
def Retrieval(grid,TP_capacity,target_block,ppo,step,grids,blocks,actions,rewards,dones,masks,probs,lookahead_num,TP_type_len):
    #grid r,c, 2+TP_type
    input_grid=classify_grid(grid,TP_capacity,target_block)
    ispossible,path,area_left,count,path_label,added_label,labeled_grid,label_num=path_finder(input_grid,target_block)
    if ispossible==False:
    ...
    rearrange_num,end_grid,step,grids,blocks,actions,rewards,dones,masks,probs=retrieval(final_grid,input_grid,grid.copy(),target_block,path,ppo,step,grids,blocks,rewards,actions,dones,masks,probs,lookahead_num,TP_type_len)
    
    return ispossible,rearrange_num,end_grid,step,grids,blocks,actions,rewards,dones,masks,probs
