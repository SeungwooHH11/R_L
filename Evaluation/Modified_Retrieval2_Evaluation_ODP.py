...
def path_finder(grid, goal):
    ...
    ispossible, dp, _, _ = backtracking_dp(grid.copy(), goal)
    if not ispossible:
...
def retrieval(final_grid,input_grid,state_grid,target_block,path,ppo,step,grids,blocks,block_lefts,block_left_num,rewards,actions,dones,masks,probs,lookahead_num,TP_type_len,rt_mod,rearrange_location_test):
    do=0
    stock=[]
    obstacles_set=[]
    for st in path:
    ...
    for ob_num in range(rearrange_num):
        ...
        if len(possible_space)==0:
            ...
            if rt_mod=='Random':
                candidate_space = np.argwhere(
                    ((mask.cpu()).reshape(input_grid.shape[0], input_grid.shape[1], 1)[:, :, 0] == 0)).T.tolist()
                random_index = np.random.randint(0, len(candidate_space))

                target_space = torch.tensor(
                    candidate_space[random_index][0] * input_grid.shape[1] + candidate_space[random_index][1])
                pr = torch.tensor(1)
            else:
...
def Retrieval(grid,TP_capacity,target_block,ppo,step,grids,blocks,block_lefts,block_left_num,actions,rewards,dones,masks,probs,lookahead_num,TP_type_len,rt_mod,rearrange_location_test):
    #grid r,c, 2+TP_type
    ...
    rearrange_num,end_grid,step,grids,blocks,actions,rewards,dones,masks,probs,block_lefts=retrieval(final_grid,input_grid,grid.copy(),target_block,path,ppo,step,grids,blocks,block_lefts,block_left_num,rewards,actions,dones,masks,probs,lookahead_num,TP_type_len,rt_mod,rearrange_location_test)
