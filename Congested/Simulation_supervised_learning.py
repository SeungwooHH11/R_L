...
class Stockyard_simulation:
    ...
    def Generate_grid(self, seed):
        ...
        grid = np.zeros((self.yard_size[0], self.yard_size[1], len(self.TP_type) + 2),
                        dtype=int)  # n*n*3 반출시간, 무게, step(블록이 배정된 스텝)
    ...
    def Run_simulation(self, simulation_day, lookahead_num, ppo, grid, total_block, total_block_encoded):
        ...
        for i in range(simulation_day):
            ...
            for e, row in enumerate(block_located):
                ...
                block_left_num -= 1
                blocks_vec = total_block_encoded[block_num + e:int(min(block_num + e + lookahead_num, max_length)),
                             :].copy()
                ...
                grid_tensor = torch.tensor(grid[:, :, :-1].reshape(1, grid.shape[0], grid.shape[1], -1),
                ...
                grid[r, c, -1] = step
                if need_retrieval:
                    ispossible, rearrange_num, end_grid, step, grids, blocks, actions, rewards, dones, masks, probs, block_lefts = Retrieval(
                        grid.copy(), len(self.TP_type) - 1, target_block.copy(), ppo, step, grids, blocks, block_lefts,
                        block_left_num, actions, rewards, dones, masks, probs, lookahead_num, len(self.TP_type), 'NOR')
            ...
            while True:
                ...
                ispossible, rearrange_num, end_grid, step, grids, blocks, actions, rewards, dones, masks, probs, block_lefts = Retrieval(
                    grid, TP_capacity, target_block, ppo, step, grids, blocks, block_lefts, block_left_num, actions,
                    rewards, dones, masks, probs, lookahead_num, TP_type_len, 'OR')
    ...
    def Train(self, train_step, eval_step, K, pr_num, batch_num, simulation_day, lookahead_num, ppo, model_dir, ASR_1,
              Random_1, BLF_1):
        eval_set = []
        ...
        for tr_step in range(train_step):
            ...
            for __ in range(pr_num):
                for ___ in range(batch_num):
                    total_rearrangement, grids, blocks, actions, rewards, dones, masks, probs, block_lefts = self.Run_simulation(
                        simulation_day, lookahead_num, ppo, grid.copy(), total_block.copy(), total_block_encoded.copy())
                    gridss.append(grids)
            ...
            for ____ in range(K):
                ave_loss= ppo.update(gridss[:, :, :, :-1], blockss, block_leftss, actionss, rewardss,
                                                      doness, maskss, probss, ep_len, tr_step, model_dir)
            ...
            if tr_step % eval_step == 0:
                ...
                print('eval',ave_rearrangement/pr_num/batch_num)
...
if __name__ == "__main__":
    ...
    ST_sim = Stockyard_simulation(yard_size=pr_size, initial_block=init_block, lam=1 / 250, weight=(1, 501),
                                  TP_type=[300, 400, 550], Block_per_Day=bpd, mod=0)
    ...
    ppo = PPO(feature_dim=4, hidden_dim=32, lookahead_block_num=1, grid_size=pr_size, learning_rate=0.001, lmbda=0.95,
              gamma=1, alpha=0.5, beta=0.01, epsilon=0.2, mod='GAT').to(device)
    '''
    ...
    '''
    history = ST_sim.Train(train_step=1000, eval_step=20, K=2, pr_num=4, batch_num=15, simulation_day=10,
                           lookahead_num=1, ppo=ppo, model_dir=model_dir, ASR_1=ASR_1, Random_1=Random_1, BLF_1=BLF_1)
